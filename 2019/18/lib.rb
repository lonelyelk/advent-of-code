...
def resolve_key_door(steps_map, keys, key_door, shift = 0, chain = [key_door])
  resolved = steps_map[key_door].each_with_object({}) do |(k_d, count), steps|
    next if chain.include?(k_d)
    if keys.include?(k_d.downcase)
      resolve_key_door(steps_map, keys, k_d, count + shift, chain + [k_d]).each do |k_r, v_r|
...
def steps_map_with_open(steps_map, key)
  ...
  # puts "RESOLVED KEY: #{resolved_key.inspect}"
  # puts "RESOLVED DOOR: #{resolved_door.inspect}"
  removed = steps_map.each_with_object({}) do |(key_door, steps), open_door_map|
    # puts "REWRITING TOP LEVEL: #{key_door} =====> #{steps.inspect}"
    # next if key_door == door
    open_door_map[key_door] = steps.each_with_object({}) do |(k_d, count), new_steps|
      # puts "INSIDE_STEP: #{k_d} => #{count}"
      # next if key_door.casecmp(k_d) == 0
      if k_d == key
...
def iterate_path(steps_map, steps, path, required_keys)
  keys = path.map { |pt| pt[0] }
  if (required_keys - keys).empty?
    puts path.inspect
    return path 
  end

  # puts path.inspect
  steps.map do |key, count|
    if keys.include?(key)
      []
    else
      # puts Time.now
      next_steps_map = steps_map_with_open(steps_map, key)
      # puts Time.now
      # puts "MAP: #{next_steps_map.inspect}; key: #{key}; keys: #{keys.inspect}"
      next_steps = resolve_key_door(next_steps_map, keys + [key], key)
      # puts Time.now
        # puts "MAP: #{next_steps_map.inspect}"
        # puts "PATH: #{path.inspect}"
        # puts "STEPS: #{steps.inspect}; NEXT: #{next_steps.inspect}"
      # if next_steps.empty?
      #   exit
      # end
      iterate_path(next_steps_map, next_steps, path + [[key, count]], required_keys)
    end
...
def solve(source_map)
  ...
  iterate_path(steps_map, steps, [], required_keys)
end
