def parse_rules(rules)
  rules.each_with_object({}) do |rule, set|
    ...
    set[md[1]] = rule.scan(/(\d+) (\w+ \w+) bags?[.,]/).each_with_object({}) { |(num, b), s| s[b] = num.to_i }
  end
end
...
def bag_found?(parsed_rules, rule, bag, starting_bag = nil)
  ...
  rule.keys.reject { |b| b == starting_bag }.any? { |b| bag_found?(parsed_rules, parsed_rules[b], bag, starting_bag)}
end

def outer_bag_count(parsed_rules, bag)
  parsed_rules.keys.reject { |b| b == bag }.select do |b|
    bag_found?(parsed_rules, parsed_rules[b], bag, b)
  end.length
end
