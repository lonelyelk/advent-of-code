...
module Year2024
  module Day09
    ...
    def problem1(input)
      drive = input.chars.each_slice(2).with_index.each_with_object([]) do |((file, space), i), acc|
    ...
    def problem2(input)
      ...
      (input.size / 2).downto(1) do |i|
        ...
        next if new_pos_size.nil? || drive[:free][new_pos_size].empty?
        next if drive[:free][new_pos_size].first > pos
